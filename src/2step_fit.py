...
# data = FreqData( maskfile='south_haze_mask_ns0128.fits')
		...
		for freq in frequencies:
			...
			tfiles = [ gmodel.galaxy.dust.filename, gmodel.galaxy.freefree.filename, 
				gmodel.galaxy.sync.filename, h.filename,# gmodel.galaxy.disc.filename,
			...
			r = map_regression( map.imap.map-gmodel.galaxy.cmb.map, tfiles, rms=map.irms.map, maskfile=data.mask.filename, return_res=True, return_chi2=True )
			chisq.append( str("%.3f" % r[3]) )
			c2 += r[3]
			# hp.mollview(r[2]*data.mask.map, min=-0.05, max=0.05, title=freq)
			# hp.graticule( dpar=20, dmer=20 )

			# hp.mollview(r[0][4] * h.map * data.mask.map, min=-0.05, max=0.05, title='Haze template @ '+freq, fig=3)
			# hp.graticule( dpar=20, dmer=20 )
			...
			flat_comp = r[0][4] + \
				gmodel.galaxy.dipole_x.map * r[0][5] + \
				gmodel.galaxy.dipole_y.map * r[0][6] + \
				gmodel.galaxy.dipole_z.map * r[0][7] + \
				gmodel.galaxy.freefree.map * r[0][1]
			...
			r = map_regression( map.imap.map-flat_comp, tfiles, rms=map.irms.map, maskfile=mfile, return_res=True, return_chi2=True )
			chisq.append( str("%.3f" % r[3]) )
			c2 += r[3]
			...
			# hp.mollview(r[0][4] * h.map * data.mask.map, min=-0.05, max=0.05, title='Haze template @ '+freq)
			# hp.graticule( dpar=20, dmer=20 )
...
# 			mnpnt['f'+freq+'_freefree'] = r[0][1]
			mnpnt['f'+freq+'_sync'] = r[0][1]
			mnpnt['f'+freq+'_haze'] = r[0][2]
# 			mnpnt['f'+freq+'_disc'] = r[0][4]
...
#  			mnpnt_er['f'+freq+'_freefree'] = r[1][1]
			mnpnt_er['f'+freq+'_sync'] = r[1][1]
			mnpnt_er['f'+freq+'_haze'] = r[1][2]
# 			mnpnt_er['f'+freq+'_disc'] = r[1][4]
...
# 		sed_er_nod = get_sed( mnpnt_er_nod )
		...
		if plot_min_sed:
			...
			fig = plt.figure(10+imodel,(9.5,7.5))
			ax = plt.subplot(111)
...
# 			plt.legend( loc='lower right',prop={'size':10})
			fig.savefig('../sed_dm_'+model+'_2step_southern_fixCMB_wDisc.png')
...
# ------ Analyze MCMC ------
	...
	#		print np.dot(v, np.array([0,1]))
		...
		#sys.exit()
		if run_2mcmc:
			bf, ch = m.sampler( p, fg_chisq, nsamples=200000, seed=4, output_tag=runtag+'_2nd',
		...
		if plot_maps:
			for i,fi in enumerate(fs):
				...
				# print pi
				bf_map = fg_model( pi )
				print fg_chisq( pi ) / np.sum(data.mask.map)
				map = getattr(data, fi)
				hp.mollview( (map.imap.map - bf_map)*data.mask.map, min=-0.025, max=0.025, title='Residuals @ '+fi+' GHz', unit='mK' )
				hp.graticule( dpar=20, dmer=20 )
		...
		if plot_sed:
			...
			fig = plt.figure(9)
			ax = plt.subplot(111)
			plt.plot( f, sed['dust'], label='dust' )
			plt.plot( f, sed['cmb'], label='cmb' )
			plt.plot( f, sed['freefree'], label='freefree' )
			plt.plot( f, sed['sync'], label='sync' )

			plt.plot( f, (f/f[0])**(-3.1)*cv*sed['sync'][0], '--', label='-3.1', color='k' )
			plt.plot( f, (f/f[0])**(-2.15)*cv*sed['freefree'][0], '--', label='-2.15', color='k' )
