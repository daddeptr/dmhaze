...
if False:
	...
	hp.write_map( dir + 'south'+str(radius)+'_haze_mask_ns0128.fits', d*m )
	hp.gnomview( d*m, rot=[0,-22.5], reso=25, min=-1, max=1)
...
for imodel, model in enumerate( model_list) :
	...
	data = Data( DMmodel=model, maskfile='ebvh_mask_ns0128.fits', raw_sync=True, regression=regression )
	gmodel = data.gmodel
	runtag = model+tag
	print runtag
...
# data = FreqData( maskfile='south_haze_mask_ns0128.fits')
		...
		#fx_mnpnt_md = OrderedDict()
		#fx_mnpnt_md_er = OrderedDict()
...
# --- Outdated, since I modified the data model with regression option
#			# --- Full run check monopole/dipole removal effect
...
# 				maskfile=data.mask.filename, return_res=True, return_chi2=True, regression=regression )
# 
# 			fx_mnpnt_md['f'+freq+'_cmb'] = r[0][0]
# 			fx_mnpnt_md['f'+freq+'_dust'] = r[0][1]
#  			fx_mnpnt_md['f'+freq+'_freefree'] = r[0][2]
# 			fx_mnpnt_md['f'+freq+'_sync'] = r[0][3]
# 			fx_mnpnt_md['f'+freq+'_haze'] = r[0][4]
# 			if inc_disc:
# 				fx_mnpnt_md['f'+freq+'_disc'] = r[0][idisc]
# 
# 			fx_mnpnt_md_er['f'+freq+'_cmb'] = r[1][0]
# 			fx_mnpnt_md_er['f'+freq+'_dust'] = r[1][1]
#   			fx_mnpnt_md_er['f'+freq+'_freefree'] = r[1][2]
# 			fx_mnpnt_md_er['f'+freq+'_sync'] = r[1][3]
# 			fx_mnpnt_md_er['f'+freq+'_haze'] = r[1][4]
# 			if inc_disc:
# 				fx_mnpnt_md_er['f'+freq+'_disc'] = r[1][idisc]
			...
			if regression == 0:
				...
				tfiles = [ gmodel.galaxy.cmb.map, gmodel.galaxy.dust.map,
					gmodel.galaxy.freefree.map, 
					gmodel.galaxy.sync.map ]
				if inc_haze:
					tfiles.append( h.map )
					ihaze = len(tfiles)-1
				if inc_disc:
					tfiles.append( gmodel.galaxy.disc.map )
					idisc = len(tfiles)-1
			else:
				info_message( 'ERROR regression 1 case not implemented. Abort.' )
				sys.exit()

			map = getattr( data.data, 'f'+freq )
			#xxx = map.imap.map
			#yyy = hp.read_map( map.imap.filename )
			#hp.mollview(xxx, min=-0.3, max=0.3)
			#hp.mollview(yyy, min=-0.3, max=0.3)
			#pl.show()
			#sys.exit()
			r = map_regression( map.imap.map, tfiles, rms=map.irms.map,
...
# 			fig.savefig(figtag+'.png')
			...
			if regression == 0:
				...
				if inc_haze:
					...
					ihaze = len( tfiles ) -1
				if not fix_ff:
					tfiles.append( gmodel.galaxy.freefree.filename )
					iff = len( tfiles ) -1
				...
				tfiles = [ gmodel.galaxy.dust.map,
					gmodel.galaxy.sync.map ]
				if not fix_ff:
					tfiles.append( gmodel.galaxy.freefree.map )
					iff = len( tfiles ) -1
				...
				if not fix_cmb:
					tfiles.append( gmodel.galaxy.cmb.map )
					icmb = len( tfiles ) -1
			...
			if plot_gnom:
				...
				if inc_haze:
					...
					ccc[ bpix] = -1.6375e30
					hp.gnomview( ccc, title='Haze: '+freq, rot=[0,-22.5], reso=25, min=-0.05, max=0.1 )
					hp.graticule(dpar=5,dmer=5)
			...
			# --- Flux from template
				flx = np.mean( r[0][ihaze]*h.map[gpix] ) * mKt2cgs[ifreq]
			...
			if not fix_ff:
				mnpnt['f'+freq+'_freefree'] = r[0][iff]
			if inc_disc:
			...
			if not fix_ff:
				mnpnt_er['f'+freq+'_freefree'] = r[1][iff]
			if inc_disc:
...
#		fx_sed_md = get_sed( fx_mnpnt_md )
#		fx_sed_md_er = get_sed( fx_mnpnt_md_er )
		...
		if plot_min_sed:
			...
			if inc_disc:
	 			plt.errorbar( f, fx_sed['disc'], yerr=fx_sed_er['disc'], label='disc FS', color='y', fmt='.-' )

#  			plt.errorbar( f, fx_sed_md['cmb'], yerr=fx_sed_md_er['cmb'], label='cmb FS-MD', color='k', fmt='d-' )
#   			plt.errorbar( f, fx_sed_md['dust'], yerr=fx_sed_md_er['dust'], label='dust FS-MD', color='b', fmt='d-' )
#   			plt.errorbar( f, fx_sed_md['sync'], yerr=fx_sed_md_er['sync'], label='sync FS-MD', color='g', fmt='d-' )
#   			plt.errorbar( f, fx_sed_md['haze'], yerr=fx_sed_md_er['haze'], label='haze FS-MD', color='m', fmt='d-' )
#   			plt.errorbar( f, fx_sed_md['freefree'], yerr=fx_sed_md_er['freefree'], label='freefree FS-MD', color='r', fmt='d-' )
# 			if inc_disc:
# 	 			plt.errorbar( f, fx_sed_md['disc'], yerr=fx_sed_md_er['disc'], label='disc FS-MD', color='y', fmt='d-' )
...
# 			plt.legend( loc='lower right',prop={'size':10})
			...
			figtag = '../sed_dm_'+model+'_2step_mdCheck_'+tag
			if fix_cmb:
				figtag = figtag + '_fixCMB'
			if not inc_disc:
				figtag = figtag + '_noDISC'
