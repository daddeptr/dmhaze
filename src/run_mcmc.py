...
tag = '_Lmask_fullFreq_x6sig_noDISC'	
...
for imodel,model in enumerate( model_list) :
	setup = Data( maskfile='south_haze_mask_ns0128.fits', DMmodel=model)
	gmodel = setup.gmodel
	data = setup.data
	bpix = np.array( setup.mask.map == 0)
	...
	if find_minimum:
		...
		mnpnt_nod = OrderedDict()
		mnpnt_er_nod = OrderedDict()
		...
		for freq in data.frequencies:
			...
			mnpnt_nod['f'+freq+'_dipole_z'] = r[0][8]

			mnpnt_er_nod['f'+freq+'_cmb'] = r[1][0]
			mnpnt_er_nod['f'+freq+'_dust'] = r[1][1]
			mnpnt_er_nod['f'+freq+'_freefree'] = r[1][2]
			mnpnt_er_nod['f'+freq+'_sync'] = r[1][3]
			mnpnt_er_nod['f'+freq+'_haze'] = r[1][4]
			mnpnt_er_nod['f'+freq+'_monopole'] = r[1][5]
			mnpnt_er_nod['f'+freq+'_dipole_x'] = r[1][6]
			mnpnt_er_nod['f'+freq+'_dipole_y'] = r[1][7]
			mnpnt_er_nod['f'+freq+'_dipole_z'] = r[1][8]
			# mnpnt_er_nod['f'+freq+'_disc'] = r[1][9]
			...
			if plot_gnom:
				ccc = r[2]*setup.mask.map
				ccc[bpix] = hp.UNSEEN
				hp.gnomview( ccc, min=-0.05, max=0.05, title=freq, fig=2, reso=30, rot=[0,-22.5])
				hp.graticule( dpar=5, dmer=5 )

				# ccc = r[0][4] * h.map
				# ccc[bpix] = hp.UNSEEN			
				# hp.gnomview( ccc, min=-0.05, max=0.05, title='Haze template @ '+freq, fig=3, reso=30)
				# hp.graticule( dpar=5, dmer=5 )
		...
		info_message(' total chi2 = '+str(c2)+','+str(c2*np.sum(setup.mask.map)))
		...
		if plot_min_sed:
			...
			fig.savefig('../sed_dm_'+model+'_noDisc_southern.png')
	...
	# ------ Run MCMC ------
	if run_mcmc:
		bf, ch = m.sampler( p, setup.fg_chisq, nsamples=200000, seed=3, output_tag=runtag,
